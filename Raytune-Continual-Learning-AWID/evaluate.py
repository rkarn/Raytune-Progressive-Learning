    ...
    def evaluate_action(self,var_list,actions,task_id):
        with tf.Graph().as_default() as g:
            ...
            with tf.name_scope("model"):
                self.x = tf.placeholder(tf.float32,shape=[None,98]) 
                self.y = tf.placeholder(tf.float32,shape=[None,17])
            ...
            for epoch in range(self.epochs):
                ...
                if epoch%4==0 or epoch==self.epochs-1:
                    print("task:%s, epoch:%s, test accuracy:%s"%(task_id, epoch, accuracy_test))
